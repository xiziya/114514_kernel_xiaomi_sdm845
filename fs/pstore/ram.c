...
static int __init ramoops_memreserve(char *p)
{
	...
	/*
	 * Keep independent persistent dmesg, console and pmsg records.  The old
	 * Xiaomi shim only reserved console and pmsg space, leaving record_size at
	 * zero, so ramoops never created a dmesg crash record.
	 */
	if (size < 3 * PAGE_SIZE)
		return 1;
	ramoops_data.mem_size = size;
	ramoops_data.mem_address = 0xB0000000;
	ramoops_data.record_size = size / 3;
	ramoops_data.console_size = size / 3;
	ramoops_data.pmsg_size = size / 3;
